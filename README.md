...
app 首次启动即启用内置预设 **「DS-zh」**（Temp 0.7 / TopP 0.95 / TopK 0 / 1M 上下文），开箱可用、**无需手动粘贴**。需要在其它客户端使用时可直接复制下面这份（`{{char}}` / `{{user}}` 为角色卡宏）：
...
> 该版本（v23，402 字符）按 **DeepSeek 实测** 定版：8/8 零格式违规；本地 4B 小模型仅作压力测试。字数与描写长度优于旧版长提示词，引用块格式零退化。
