...
app 首次启动即启用内置预设 **「简体中文 RP」**，开箱可用、**无需手动粘贴**。需要在其它客户端使用时可直接复制下面这份（`{{char}}` / `{{user}}` 为角色卡宏）：
...
> 该版本按**本地小模型**（4B 级，如 Gemma-4-E4B）的遵循能力做过精简与采样实测：输出字数、单段描写长度、情绪解说三项均优于旧版长提示词，而引用块格式零退化。
